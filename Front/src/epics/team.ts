...
export const dealJoinGroup = (id: string) => ({
  type: DEAL_JOINGROUP,
  data: id
});
...
export const fetchTeam = (action$: EpicAction) =>
  action$.ofType(FETCH_TEAM).mergeMap((action: Action) => {
    return (
      fetch
        .get(teamList)
...
export const EsendApply = (action$: EpicAction) =>
  action$.ofType(SEND_APPLY).mergeMap((action: Action) => {
    return (
      fetch
        .post(sendApply, action.data)
        .map((response: Response) => {
          if (response.state.code === 1) {
...
export const ErejectJoinGroup = (action$: EpicAction) =>
  action$.ofType(REJECT_JOINGROUP).mergeMap((action: Action) => {
    return (
      fetch
        .post(rejectJoinGroup, action.data)
        .map((response: Response) => {
          if (response.state.code === 1) {
            successMsg(response.state.msg);
...
export const EallowedJoinGroup = (action$: EpicAction) =>
  action$.ofType(ALLOWED_JOINGROUP).mergeMap((action: Action) => {
    return (
      fetch
        .post(allowedJoinGroup, action.data)
        .map((response: Response) => {
          if (response.state.code === 1) {
            successMsg(response.state.msg);
            socket.emit("userJoinTeam", {
...
export const EremoveGroupMember = (action$: EpicAction) =>
  action$.ofType(REMOVE_GROUPMEMBER).mergeMap((action: Action) => {
    return (
      fetch
        .post(removeGroupMember, action.data)
        .map((response: Response) => {
          if (response.state.code === 1) {
            successMsg(response.state.msg);
...
export const EinvitedGroupMember = (action$: EpicAction) =>
  action$.ofType(INVITED_GROUPMEMBER).mergeMap((action: Action) => {
    return (
      fetch
        .post(invitedGroupMember, action.data)
        .map((response: Response) => {
          if (response.state.code === 1) {
            successMsg(response.state.msg);
            return nothing();
