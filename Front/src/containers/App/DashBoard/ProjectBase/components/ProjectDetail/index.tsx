...
> {
  ...
  showAllProjectMessages = () => {
    this.setState({
      allProjectMessages: true
    });
  };
  render() {
    return (
        ...
        <Tabs defaultActiveKey="1" onTabClick={this.selectTab}>
          <TabPane tab="项目简介" key="1">
            <div className="exportMe">
              <Tooltip placement="right" title={"导出项目"}>
                <Icon
                  type="right-circle"
                  onClick={() => this.props.showExportProject()}
                />
              </Tooltip>
            </div>
              ...
              <div className="proejctGroup title">
                ...
                <ul className="userList">
                  {this.props.data.teamMember.length > 0
                    ? this.props.data.teamMember.map(
                        (user: TeamMember, key: number) => {
                          return (
                            <li key={key}>
                              <Popover
                                content={
            ...
            <div className="proejctDesc title">
              ...
              <EditableCell
                ...
                onChange={this.changeProjectDesc(
                  this.props.data.projectDesc,
                  this.props.data._id
                )}
              />
            </div>
            <div className="proejctVersion title">
              ...
              <EditableCell
                ...
                onChange={this.changeProjectVersion(
              ...
              <div className="backOperate">
                <EditableCell
                  ...
                  onChange={this.changeTransferUrl(
                    this.props.data.transferUrl,
                    this.props.data._id
                  )}
                />
                {this.props.data.status === "mock" ? (
                  <Button
                    onClick={() => this.toggleTransfer(this.props.data._id)}
                  >
                ...
                <Button
                  onClick={() =>
                    this.props.showAutoCheckVisible(this.props.data._id)
                  }
                >
          ...
          <TabPane tab="项目动态" key="3">
            <div className="projectTimeline">
              <Timeline
                pending={
                  this.props.messages.length < 80 ? (
                    <div>已经是全部信息了~</div>
                  ) : (
                    <div
                      className="showAllProjectMessages"
                      onClick={this.showAllProjectMessages}
                    >
                      查看所有动态
                    </div>
                  )
              ...
              >
                {this.props.messages.map((item: Message, index: number) => {
                  if (!this.state.allProjectMessages) {
                    if (index < 80) {
                      return (
                        <Timeline.Item
